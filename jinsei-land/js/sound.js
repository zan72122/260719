...
const S = (() => {
  ...
  const api = {
    ...
    thunder() {
      hiss(1.1, 0.25, 0, 300);
      blip('sine', 90, 45, 1.0, 0.3);
      blip('sine', 70, 40, 0.8, 0.2, 0.25);
    },

    /* すーっと おちる（あなに ぽとん） */
    fallDown() {
      blip('sine', 1300, 140, 0.9, 0.26);
      blip('sine', 150, 80, 0.2, 0.22, 0.9);
      hiss(0.15, 0.15, 0.9);
    },
    ...
    geyser() {
      hiss(1.0, 0.3, 0, 500);
      blip('sine', 200, 1200, 0.9, 0.2);
      blip('sine', 500, 1600, 0.6, 0.12, 0.2);
    },

    /* ジェットコースターの わーい！ */
    scream() {
      blip('sine', 500, 1100, 0.5, 0.16);
      blip('sine', 1100, 600, 0.5, 0.14, 0.5);
      blip('sine', 700, 1300, 0.45, 0.12, 1.0);
      hiss(1.6, 0.1, 0, 1800);
    },
    ...
    wind() {
      hiss(1.6, 0.28, 0, 400);
      blip('sine', 180, 420, 0.8, 0.1);
      blip('sine', 420, 240, 0.8, 0.1, 0.8);
    },

    /* ねずみの ちゅーちゅー */
    squeak() {
      blip('sine', 2300, 2800, 0.09, 0.15);
      blip('sine', 2600, 2100, 0.11, 0.13, 0.13);
      blip('sine', 2400, 2900, 0.09, 0.12, 0.28);
    },

    /* おかいもの チーン！ */
    kaching() {
      tone(N.E6, 0.15, 0.25);
      tone(N.C6, 0.3, 0.22, 0.1);
      hiss(0.2, 0.08, 0.1, 5000);
      tone(N.G6, 0.35, 0.18, 0.22);
    },

    /* けっこんしきの かね */
    bells() {
      const seq = [[N.C6, 0], [N.E6, 0.22], [N.G6, 0.44], [N.C6, 0.8], [N.E6, 1.02], [N.G6, 1.24]];
      seq.forEach(([f, d]) => { tone(f, 0.5, 0.2, d); tone(f * 2, 0.4, 0.06, d); });
    },

    /* あかちゃんの ぷくぷく */
    baby() {
      blip('sine', 700, 1000, 0.12, 0.16);
      blip('sine', 900, 1200, 0.12, 0.16, 0.18);
      blip('sine', 1100, 800, 0.16, 0.14, 0.36);
    },

    /* せいちょう！レベルアップ */
    levelup() {
      const seq = [N.C5, N.E5, N.G5, N.C6, N.E6];
      seq.forEach((f, i) => tone(f, 0.22, 0.24, i * 0.09, 'square'));
      tone(N.G6, 0.5, 0.2, 0.5);
      hiss(0.4, 0.07, 0.45, 4000);
    },
